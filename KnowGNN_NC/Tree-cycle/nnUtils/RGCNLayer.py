...
from nnUtils.abstract_gnn import AbstractGNN
...
class RGCNLayer(MessagePassing):
    ...
    def message(self, x_j, x_i, edge_type, message_scale, message_replacement):    #在这里就是做了对边的mask操作，若有mask（gate），则根据mask后的边的信息执行GNN
        ...
        #
        # if message_scale is not None:   #根据mask，用baselines替换边的信息
        ...
        #         else:
        #             basis_messages = basis_messages + (1 - message_scale).unsqueeze(-1) * message_replacement.unsqueeze(0)
